...
function getPeilingwijzerGrowth(callback) {
  ...
  q.exec(function(error, result) {
    if(error) {
      ...
      var endDateStart = moment(result[0].date).format('YYYY-MM-DDT00:00:00.000Z');
      var endDateEnd = moment(endDateStart).format('YYYY-MM-DDT23:59:59.000Z');

      var startDateStart = moment(result[0].date).subtract(14, 'days').format('YYYY-MM-DDT00:00:00.000Z');
      var startDateEnd = moment(startDateStart).format('YYYY-MM-DDT23:59:59.000Z');
      ...
      Peilingwijzer.find(query).limit(2).exec(function(error, results) {
        if(error) {
          ...
          var items = {
            startDate: moment(startDateStart).format('D MMM'),
            endDate: moment(endDateStart).format('D MMM'),
            items: []
          };

          for(var partyName in results[0].seats) {

            items.items.push({
              party: partyName,
              startWeight: results[0].seats[partyName],
              endWeight: results[1].seats[partyName],
              growth: (results[1].seats[partyName] - results[0].seats[partyName]).toFixed(1)
            })
          }
...
function getAveragePublicationWeightWithinTimeframe(startTime, endTime, callback) {
  ...
  var endHHmm = moment().subtract(1, 'hours').format('HH') + moment().format('mm');
  var startHHmm = moment().subtract(2, 'hours').format('HH') + moment().format('mm');
...
function getAveragesPerDay(callback) {
  ...
  async.series({
    ...
    Averages: function(itemCallback) {
      getAllTimeAverages(function(error, results) {
        if(error) return itemCallback(error);
        results.map(function(result) {
          averages[result._id.publisherLocation] = result.averageWeight;
        })
        itemCallback(null);
      })
    },
      ...
      getLocationAveragesPerDay('Amsterdam', function(error, results) {
        ...
        var items = results.map(function(result) {
          return {
            ...
            averageWeight: result.averageWeight,
            deviationPercentage: (result.averageWeight / averages.Amsterdam * 100 ) - 100
          }
        })
      ...
      getLocationAveragesPerDay('Paris', function(error, results) {
        ...
        var items = results.map(function(result) {
          return {
            ...
            year: result._id.year,
            averageWeight: result.averageWeight,
            deviationPercentage: (result.averageWeight / averages.Paris * 100 ) - 100
          }
        })
        items.sort(function(a, b) {
          return a.date - b.date
      ...
      getLocationAveragesPerDay('Berlin', function(error, results) {
        ...
        var items = results.map(function(result) {
          return {
            ...
            year: result._id.year,
            averageWeight: result.averageWeight,
            deviationPercentage: (result.averageWeight / averages.Berlin * 100 ) - 100
          }
        })
        items.sort(function(a, b) {
          return a.date - b.date
...
function getAllTimeAverages(callback) {
  var query = [
    {
      "$match": {
        "date": {
